...
#        #battery time-dependency (SOC): decision making on whether to charge (a=1) or discharge battery (a=-1)
       ...
       # elif (SOC[j] > 0.1) and (externalCost > ext_cost_average): #for multiple hours: ext_cost[i]
        ...
        co2_next = (abs(pvIndex*net.res_sgen.at[1, 'p_kw'])+abs(windIndex*net.res_sgen.at[2, 'p_kw'])+abs(extIndex*extPower))/(abs(net.res_sgen.at[1, 'p_kw'])+abs(net.res_sgen.at[2, 'p_kw'])+abs(extPower))
        co2.append(co2_next)
