...
class Autocar:
    ...
    def optimize_ChargingTime(self, grid, grid_chargingStation, avg_velocity, t_charging_max):
        ...
        max_distance = Autocar.calc_energy_need(self)[
                           0] / self.powerkm;  # grid has to be in reachable distace according to state of charge in km
        print('estimate reachable distance in km:', max_distance);
        lat = 0;
        long = 1;  # defined by grid parameter -->has to be changed if information will be delivered in a different way
        ...
        for n in range(len(grid)):
            ...
            t_reach = Autocar.find_distance_km(grid[n][lat], grid[n][long], self.AC_location[0],
                                               self.AC_location[1]) / avg_velocity;
