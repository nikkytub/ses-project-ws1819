...
def createprosumer(month,day,hour):
    ...
    for j in range(0,len(names)):
        ...
        if prosumer_type == 'p2':
            ...
            number_chargingstation2=1
            charging_station1=charging_station_office1[index1]
            charging_station2=charging_station_office2[index1]
        ...
        if ((SOC[j] < 1) and (ext_cost[index1] <= ext_cost_average)): #for multiple hours: ext_cost[i]
            a=1
        elif (SOC[j] > 0) and (ext_cost[index1] > ext_cost_average): #for multiple hours: ext_cost[i]
            a=-1
        else: a=0
        
        print('a', a)
        #create battery (SOC is timedependent), momentary real power of the storage (positive for charging, negative for discharging)
        battery = pp.create_storage(net, bus6, p_kw=a*1/10*c_battery, max_e_kwh=c_battery, controllable=True, max_p_kw = 1/4*c_battery, min_p_kw = -1/4*c_battery, index=3)
   ...
   # with open('result_load_school_zoo_garden.csv', 'r', encoding='UTF-8') as f:
    ...
    with open('chargingStation.csv', 'r', encoding='UTF-8') as f:
        ...
        for line in f:
            ...
            charging_station_office2.append(float(row[3]))
            charging_station_office1.append(float(row[2])) 
